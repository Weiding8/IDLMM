...
data = data[data['cell_line'] == 'HCC366']
...
plt.xlabel('Drug', fontsize=32, labelpad=-12)
...
plt.ylim(0.35, 0.98)
plt.yticks(np.arange(0.4, 0.81, 0.1), fontsize=26)
...
# 移动标签位置（向右偏移)
ax.set_xticks(np.arange(len(drugs)) + 0.6)  # 0.2是一个示例偏移量，调整此值以获得所需的效果
...
# 添加标题框
rect_title = plt.Rectangle((-1, 0.895), len(drugs)+1, 2, linewidth=3, edgecolor='black', facecolor='#D0D7DB')
plt.gca().add_patch(rect_title)
plt.text(2.8, 0.918, r'HCC366',
...
# plt.show()
plt.savefig('../fig/HCC366_bio.png', bbox_inches='tight')